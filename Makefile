...
LIBOGG_ARCHIVE := $(LIBOGG_DIR).tar.xz
...
LIBVORBIS_ARCHIVE := $(LIBVORBIS_DIR).tar.xz
...
AUTOTOOLS_CONFIGURE = mkdir -p $(@D)/_build && cd $(@D)/_build && ../configure --host=i686-w64-mingw32 --prefix=$(PREFIX_DIR) CFLAGS="-march=i686 -I$(PREFIX_DIR)/include" LDFLAGS="-L$(PREFIX_DIR)/lib"
...
$(OPENRCT2_DIR)/extracted:
	git clone --depth 1 --branch v0.4.12 https://github.com/OpenRCT2/OpenRCT2
...
$(LIBOGG_ARCHIVE):
	wget https://downloads.xiph.org/releases/ogg/$@

$(LIBOGG_DIR)/configured: $(LIBOGG_DIR)/extracted
	$(CMAKE_CONFIGURE)
	touch $@
...
$(LIBVORBIS_ARCHIVE):
	wget https://downloads.xiph.org/releases/vorbis/$@

$(LIBVORBIS_DIR)/configured: $(LIBVORBIS_DIR)/extracted $(LIBOGG_DIR)/installed
	$(CMAKE_CONFIGURE)
	touch $@
...
$(NLOHMANNJSON_DIR)/configured: $(NLOHMANNJSON_DIR)/extracted
	$(CMAKE_CONFIGURE) -DJSON_BuildTests=OFF
	touch $@
...
$(OPENSSL_DIR)/configured: $(OPENSSL_DIR)/extracted
	mkdir -p $(@D)/_build && cd $(@D)/_build && CFLAGS="-march=i686" ../Configure mingw --prefix="$(PREFIX_DIR)" --cross-compile-prefix=i686-w64-mingw32- no-tests no-docs no-dso no-module $(if $(STATIC),no-shared,) 386 -D_WIN32_WINNT=0x501
	touch $@
